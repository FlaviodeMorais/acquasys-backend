...
export class TelegramBotService extends EventEmitter {
  ...
  constructor() {
    ...
    if (!this.botToken || !this.chatId) {
      console.warn(
        "⚠️ Telegram bot não configurado - verifique TELEGRAM_BOT_TOKEN e TELEGRAM_CHAT_ID"
      );
      return;
    }
  ...
  private formatMessage(alert: AlertData): string {
    const alertIcon = { warning: "⚠️", critical: "🚨", info: "ℹ️" }[
      alert.alertType
    ];
  ...
  async sendAlert(alert: AlertData): Promise<boolean> {
    ...
    try {
      ...
      if (response.data.ok) {
        console.log(
          `✅ Alerta Telegram enviado: ${alert.alertType} - ${alert.message}`
        );
        return true;
      } else {
  ...
  async testConnection(): Promise<boolean> {
    ...
    try {
      ...
      console.log(
        `🤖 Bot conectado: ${response.data.result.first_name} (@${response.data.result.username})`
      );
      return response.data.ok;
    } catch (error: any) {
  ...
  private async pollUpdates(): Promise<void> {
    ...
    try {
      ...
      if (error.response?.status === 409) {
        console.warn(
          "⚠️ Outro processo do bot Telegram já está ativo (erro 409). Polling pausado."
        );
    ...
    try {
      switch (command) {
        ...
        case "/manual":
          ...
          await this.sendMessage(
            chatId,
            "🔧 <b>Modo Manual ativado!</b>\nUse /ligar e /desligar."
          );
          break;
        ...
        case "/automatico":
          ...
          await this.sendMessage(chatId, "🤖 <b>Modo Automático ativado!</b>");
          break;

        case "/ligar":
          this.emit("pumpControl", { action: "on", userId });
          await this.sendMessage(chatId, "🟢 Bomba ligada manualmente.");
          break;

        case "/desligar":
          this.emit("pumpControl", { action: "off", userId });
          await this.sendMessage(chatId, "🔴 Bomba desligada manualmente.");
          break;
